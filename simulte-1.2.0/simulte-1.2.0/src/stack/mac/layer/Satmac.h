...
#include "stack/mac/packet/SatmacPacket_m.h"
...
//此类实现数据帧和管理帧的数据包分段和重传策略。
//它使用ns3:：MacLow和ns3:：ChannelAccessManager帮助程序类分别发送数据包并决定何时发送。
//数据包存储在ns3:：WifiMacQueue中，直到可以发送为止。
//目前实现的策略使用一个简单的碎片阈值：任何大于该阈值的数据包都被碎片化为大小小于该阈值的碎片。
//重传策略也很简单：每个数据包都会被重传，直到它被成功传输，或者它已经被重传到ssrc或slrc阈值。

class TdmaSatmac : public LteMacBase
{
public:
  ...
  /**
   */
  simtime_t GetSlotTime (void) const;
  /**
   */
  simtime_t GetGuardTime (void) const;
  /**
   */
  //DataRate GetDataRate (void) const;
  /**
   */
  simtime_t GetInterFrameTimeInterval (void) const;
  ...
  simtime_t CalculateTxTime (Ptr<const Packet> packet);
    ...
    double getFrameadjCutRatioEhs() const {
        return frameadj_cut_ratio_ehs_;
    }

    void setFrameadjCutRatioEhs(double frameadjCutRatioEhs) {
        frameadj_cut_ratio_ehs_ = frameadjCutRatioEhs;
    }

    double getFrameadjCutRatioThs() const {
        return frameadj_cut_ratio_ths_;
    }

    void setFrameadjCutRatioThs(double frameadjCutRatioThs) {
        frameadj_cut_ratio_ths_ = frameadjCutRatioThs;
    }

    double getFrameadjExpRatio() const {
        return frameadj_exp_ratio_;
    }

    void setFrameadjExpRatio(double frameadjExpRatio) {
        frameadj_exp_ratio_ = frameadjExpRatio;
    }

    std::string getTraceOutFile() {
        return m_traceOutFile;
    }

    void setTraceOutFile(std::string traceOutFile) {
        m_traceOutFile = traceOutFile;

//      AsciiTraceHelper ascii;
//      m_log_lpf_stream = ascii.CreateFileStream(m_traceOutFile);
    }
    ...
    void setNodePtr(Ptr<Node> nodePtr) {
        m_nodePtr = nodePtr;
    }

    int getVemacMode() const {
        return vemac_mode_;
    }

    void setVemacMode(int vemacMode) {
...
private:
  ...
  static double GetDefaultFrameadjCutRatioEhs();
  static double GetDefaultFrameadjCutRatioThs();
  static double GetDefaultFrameadjExpRatio();
...
//  virtual void DoInitialize (void);
  ...
  /**
   * The `m_BCHTrace` trace source. Track the transmission of v2x message (announce)
   */
  TracedCallback<> m_BCHTrace;
...
Ptr<MobilityModel> Mobility_;
bool location_initialed_;
bool direction_initialed_;
Coord last_location_;
Coord curr_location_;
int direction_;
