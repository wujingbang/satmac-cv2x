...
void LteMacVUeMode4::handleMessage(cMessage *msg)
{
    ...
    if (incoming == down_[IN]) // down_ --> MAC <--> PHY
    {
    ...
    else if (incoming == up_[IN])
    {
        if (strcmp(pkt->getName(), "newDataPkt")== 0)
        {
            ...
            receivedTime_ = NOW;
            simtime_t elapsedTime = receivedTime_ - lteInfo->getCreationTime(); //消逝的时间
            simtime_t duration = SimTime(lteInfo->getDuration(), SIMTIME_MS);   //持续时间
...
void LteMacVUeMode4::handleSelfMessage()
{
    ...
    // 从所有 harqrx 缓冲区中提取 pdus 并将它们传递给 unmaker
...
void LteMacVUeMode4::macHandleSps(cPacket* pkt)
{
     ...
     * 1. 需要在自我消息部分确保如果在任何时候我们有一个没有分配子通道的调度授权，我们必须等待
     * 2. Need to pick at random from the SPS list of CSRs
     * 2. 需要从 CSRs 的 SPS 列表中随机挑选
     * 3. Assign the CR
     * 4. return
     */
    ...
    // 开始的子信道到结束的子信道 因为可能占用多个连续的子信道
    int initiailSubchannel = std::get<2>(selectedCR); //感觉像是选中的 CSR 在其所在帧的索引
    int finalSubchannel = initiailSubchannel + mode4Grant->getNumSubchannels(); // Is this actually one additional subchannel?

    // Emit statistic about t0he use of resources, i.e. the initial subchannel and it's length.
    ...
    if (adjacencyPSCCHPSSCH_){
        ...
        totalGrantedBlocks += 2; // SCI 已经占了2个RB
        int initialBand;
        if (initiailSubchannel == 0){ // 开始的子信道是0号
            ...
            initialBand = (numSubchannels_ * 2) + (initiailSubchannel * (subchannelSize_ - 2)); // subchannelSize_ 是包含SCI的2个RB在内的子信道大小
            // 所以 subchannelSize_ - 2 是在不邻接模式中的子信道中 band 的数量
        }
...
void LteMacVUeMode4::macGenerateSchedulingGrant(double maximumLatency, int priority)
{
    ...
    sendLowerPackets(phyGrant); // 向物理层发送 Grant 包
