...
void LtePhyVUeMode4::handleUpperMessage(cMessage* msg)
{
    ...
    if (lteInfo->getFrameType() == GRANTPKT)
    {
        ...
        if (grant->getTotalGrantedBlocks() == 0){
            ...
            // 生成 CSR 向量并将其发送到 MAC 层
            computeCSRs(grant); // 其中 send 函数向 mac 层发送 CSRs
            delete lteInfo;
            delete grant;
        }
        else
...
void LtePhyVUeMode4::computeCSRs(LteMode4SchedulingGrant* &grant) {
    ...
    // 将数据包向上发送到 MAC 层，它将选择 CSR 和重传（如果已指定）
    ...
    candidateResourcesMessage->setCSRs(optimalCSRs); //最佳的CSRs
